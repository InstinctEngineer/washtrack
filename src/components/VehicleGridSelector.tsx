...
export function VehicleGridSelector({
  ...
  const [existingWashEntries, setExistingWashEntries] = useState<
    Map<string, { id: string; employee_id: string; employee_name: string }>
  >(new Map());
  ...
  const handleSubmit = async () => {
    ...
    const vehiclesToAdd = Array.from(selectedVehicles).filter((vehicleId) => !existingWashEntries.has(vehicleId));
    ...
    const vehiclesToRemove = Array.from(existingWashEntries.entries())
      .filter(([vehicleId, entry]) => !selectedVehicles.has(vehicleId) && entry.employee_id === employeeId)
            ...
            {vehicles.map((vehicle) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={isSelected ? "drop-shadow-sm" : ""}>{vehicle.vehicle_number}</span>
