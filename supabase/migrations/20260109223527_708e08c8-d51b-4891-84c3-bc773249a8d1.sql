...
-- Recreate with new return columns for parent_company and rate_type
CREATE OR REPLACE FUNCTION public.get_report_data(p_start_date date, p_end_date date, p_client_ids text[] DEFAULT NULL::text[], p_location_ids text[] DEFAULT NULL::text[], p_work_type_ids text[] DEFAULT NULL::text[])
 RETURNS TABLE(client_id text, client_name text, client_email text, client_terms text, client_class text, client_is_taxable boolean, client_tax_jurisdiction text, client_tax_rate numeric, client_parent_company text, location_id text, location_name text, work_type_id text, work_type_name text, work_type_rate_type text, frequency text, rate numeric, total_quantity numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
